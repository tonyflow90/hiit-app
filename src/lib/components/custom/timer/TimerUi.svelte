<script>
  ...
  import Countdown from "$components/custom/timer/Countdown.svelte";
  import TimerUiBackground from "$components/custom/timer/TimerUiBackground.svelte";
...
>
  <div class="flex flex-col items-center">
    ...
    {#if !running}
      <Button
        variant="ghost"
        class="hover:bg-transparent w-14 h-14 mt-4 p-0 m-0"
        on:click={run}
      >
    ...
    {:else if running && paused}
      <div class="flex flex-row mt-4">
        <Button
          variant="ghost"
          class="hover:bg-transparent w-14 h-14 p-0 m-0"
          on:click={resume}
        >
          <Play class="w-14 h-14 ml-2" />
        </Button>
